...
// -------------------------- 通用回调实现 --------------------------
// TestServerHandler 实现TestCreateTcpTlsServerProcessFunc接口
...
// TestClientHandler 实现TcpTlsClientProcessFunc接口
...
func TestTCP_NoTLS(t *testing.T) {
	...
	server := NewTcpTlsServer(serverHandler, WithReadWriteTimeout(true, 10*time.Second, 10*time.Second))
	...
	client := NewTcpTlsClient(clientHandler, WithClientReadWriteTimeout(10*time.Second, 10*time.Second))
...
func TestTLS_OneWay(t *testing.T) {
	...
	server := NewTcpTlsServer(serverHandler, WithServerTLS(tlsServerCfg))
	...
	client := NewTcpTlsClient(clientHandler, WithClientTLS(tlsClientCfg), WithClientReadWriteTimeout(10*time.Second, 10*time.Second))
...
func TestEdgeCases(t *testing.T) {
	...
	serverHandler := NewTestServerHandler()
	server := NewTcpTlsServer(serverHandler, WithReadWriteTimeout(true, time.Duration(1)*time.Second,
	...
	t.Run("空数据发送", func(t *testing.T) {
		client := NewTcpTlsClient(NewTestClientHandler())
...
func BenchmarkTCP_Throughput(b *testing.B) {
	...
	serverHandler := NewTestServerHandler()
	server := NewTcpTlsServer(serverHandler)
	...
	clientHandler := NewTestClientHandler()
	client := NewTcpTlsClient(clientHandler)
	err := client.Start(serverAddr)
	if err != nil {
		b.Fatal("客户端连接失败:", err)
	}
