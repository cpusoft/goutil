...
func DecodeIPAddressBlock(data []byte) ([]IPCertificateInformation, error) {
	...
	fmt.Println("data:", convert.PrintBytesOneLine(data))
	...
	for _, ipaddrfam := range blk {
		if ipaddrfam.IPAddressChoice.Tag == asn1.TagNull {
			fmt.Println("asn1.TagNull:", ipaddrfam.IPAddressChoice.Tag)
			var family uint8
			...
			fmt.Println("asn1.TagNull:", family)
			...
			fmt.Println("asn1.TagSequence ipaddress or min/max:", ipaddrranges, err)
			if err != nil {
				return ipaddresses, err
			}
			...
			for _, ipaddrrange := range ipaddrranges {
				if ipaddrrange.Tag == asn1.TagBitString {
					var addrRange asn1base.BitString
					//_, err := asn1.Unmarshal(ipaddrrange.Bytes, &addrRange)
					//_, err := asn1.Unmarshal(ipaddrrange.FullBytes, &addrRange)
					addrRange, err := asn1base.ParseBitString(ipaddrrange.Bytes)
					fmt.Println("ipaddrranges asn1.TagBitString, fullBytes:", convert.PrintBytesOneLine(ipaddrrange.FullBytes),
						"    bytes:", convert.PrintBytesOneLine(ipaddrrange.Bytes),
						" addrRange:", addrRange, err)
					if err != nil {
						return ipaddresses, err
					}
					...
					fmt.Println("ipaddrranges asn1.TagBitString  DecodeIP:", a, ipaddresses)
					...
					fmt.Println("ipaddrranges asn1.TagSequence:", a, b, err)
				}
			}
		}
	}
	fmt.Println("ipaddresses:", jsonutil.MarshalJson(ipaddresses))
