	...
	for i := 1; i < len(data); i++ { // 从第二个字节开始遍历，避免重复处理
		...
		if f >= 0x80 {
			...
			if tmp > math.MaxUint32>>7 {
				belogs.Debug("DecodeOid(): potential overflow when parsing OID byte")
				tmp = 0
				continue
			}
			tmp = tmp<<7 + (f & 0x7f)
...
func DecodeInteger(data []byte) (ret uint64) {
	...
	// 检查溢出风险
	if len(data) > 8 {
		belogs.Debug("DecodeInteger(): input too long, potential overflow")
		return 0
	}
	...
	for _, i := range data {
		...
		if ret > (math.MaxUint64-uint64(i))/256 {
			belogs.Debug("DecodeInteger(): integer overflow detected")
			return 0
		}
