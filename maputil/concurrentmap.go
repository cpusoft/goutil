...
func create[K comparable, V any](sharding func(key K) uint32, capacity int) ConcurrentMap[K, V] {
	m := ConcurrentMap[K, V]{
		sharding: sharding,
	...
	for i := 0; i < SHARD_COUNT; i++ {
		if capacity > 0 {
			m.shards[i] = &ConcurrentMapShared[K, V]{items: make(map[K]V, capacity)}
		} else {
...
// if capacity is 0, will no use capacity
func New[V any](capacity int) ConcurrentMap[string, V] {
	return create[string, V](fnv32, capacity)
}

// Creates a new concurrent map.
func NewStringer[K Stringer, V any](capacity int) ConcurrentMap[K, V] {
	return create[K, V](strfnv32[K], capacity)
}

// Creates a new concurrent map.
func NewWithCustomShardingFunction[K comparable, V any](sharding func(key K) uint32, capacity int) ConcurrentMap[K, V] {
	return create[K, V](sharding, capacity)
}
